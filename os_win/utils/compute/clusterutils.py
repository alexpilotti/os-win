...
class ClusterUtilsBase(baseutils.BaseUtils):
    ...
    def _get_cluster_nodes(self):
        return self._conn_cluster.MSCluster_ClusterToNode(
            Antecedent=self._cluster.path_())

    def _get_vm_groups(self):
        resources = self._conn_cluster.MSCluster_ClusterToResourceGroup(
...
class ClusterFailoverMonitor(ClusterUtilsBase):
    ...
    def monitor(self, callback):
        ...
        try:
            ...
            match = re.search('Virtual Machine (.*)', wmi_object.Name)
            if match:
